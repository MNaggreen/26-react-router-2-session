...
const Menu = () => {
    return (
    <nav>
        <NavLink className={({ isActve }) => isActive ? 'activeLink' : 'link'} 
        to="." end>{/* деструктуризируем значение как переменную для функции
        ...
        <NavLink className={({ isActve }) => isActive ? 'activeLink anotherLink' : 'link'}  to="/about">About</NavLink>
        <NavLink style={({ isActve }) => isActive ? {color: 'lightyellow', textDecoration: 'none'} : {}}  to="/contacts">Contacts</NavLink>
